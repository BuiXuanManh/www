...
public class CustomerResources {
    ...
    public Response findById(@PathParam("id") Long id) {
        Customer customerDTO = customerService.findById(id);
        if (customerDTO != null) {
...
//    http://localhost:8080/api/customers
    ...
    @Produces("application/json")
    public Response create(Customer customerDTO) {
        boolean save = customerService.create(customerDTO);
        if (save) {
...
//    http://localhost:8080/api/customers/{id}
    ...
    @Consumes("application/json")
    @Produces("application/json")
    public Response update(@PathParam("id") Long id, Customer customerDTO) {
